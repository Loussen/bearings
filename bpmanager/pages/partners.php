...
if($_POST) // Add && edit
{
    ...
    while($row=mysqli_fetch_assoc($sql))
    {
        ...
        {
            mysqli_query($db,"update $do set title='$title',link='$link',updated_at='$time' where lang_id='$row[id]' $add_where");
        }
        else
        {
            mysqli_query($db,"insert into $do values (0,'$title','$link','','$last_order','$active', '$row[id]', '$auto_id', '$time',0) ");
        }
    }
    ...
    if($image_tmp!="")
    {
        ...
        if($image_access==true)
        {
            ...
            if($edit>0)
            {
                ...
                @unlink("../images/partners/".$old_image_name['image_name']);
            }

            move_uploaded_file($image_tmp,'../images/partners/'.$image_upload_name);

            compress('../images/partners/'.$image_upload_name, '../images/partners/'.$image_upload_name, 80);
...
if($delete>0 && mysqli_num_rows(mysqli_query($db,"select id from $do where auto_id='$delete' "))>0)
{
    ...
    @unlink('../images/partners/'.$data["image_name"]);
...
<div class="onecolumn">
    ...
    <div class="content">
        ...
        <form action="index.php?do=<?php echo $do; ?>&page=<?php echo $page; if($edit>0) echo '&edit='.$edit; ?>" method="post" id="form_login" name="form_login" enctype="multipart/form-data">
            ...
            echo 'Link : <br />
                  <input type="url" name="link" value="'.$information["link"].'" style="width:800px" />
                  <br /><br />';

            if($information["image_name"]!="" && $edit>0)
            {
                $image='<ul class="media_photos" style="margin-left:450px;margin-top:-25px; margin-right: 40px;"><li><a rel="slide" href="../images/partners/'.$information["image_name"].'?rand='.rand(0,10000).'" title="">
                    Current image : <img src="../images/partners/'.$information["image_name"].'?rand='.rand(0,10000).'" alt="" width="120" /></a>
                    <br class="clear" />
        ...
        while($row=mysqli_fetch_assoc($sql))
        {
            echo '<tr>
					...
					<td><ul class="media_photos" style="padding:0;margin-top:0px;margin-bottom:-40px"><li><a rel="slide" href="../images/partners/'.$row["image_name"].'" title=""><img src="../images/partners/'.$row["image_name"].'?rand='.rand(0,10000).'" width="100" height="60"></a></li></ul></td>
					<td>
