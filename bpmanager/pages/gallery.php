<?php
$alboms=intval($_GET["alboms"]);
if($alboms==0)
{
    $alboms=mysqli_fetch_assoc(mysqli_query($db,"select * from alboms order by order_number asc"));
    $alboms=$alboms["auto_id"];
}
...
if($edit>0 && mysqli_num_rows(mysqli_query($db,"select id from alboms where auto_id='$edit' "))==0)
...
if($_POST["add_new_image"])
{
    ...
    foreach($image_tmp_names as $image_tmp_name)
    {
        ...
        if($image_access==true)
        {
            ...
            mysqli_query($db,"insert into $do values (0,'$time','$image_upload_name','$alboms') ");

            $last_id=mysqli_insert_id($db);
            move_uploaded_file($image_tmp_name,'../images/gallery/'.$image_upload_name);
            chmod('../images/gallery/'.$image_upload_name, 0755);
...
elseif($delete>0 && mysqli_num_rows(mysqli_query($db,"select id from $do where id='$delete' "))>0)
{
    ...
    @unlink('../images/gallery/'.$data['image_name']);
...
<div class="onecolumn">
    ...
    <div class="content">
        ...
        ?>
        <!-- Content start-->
        <form action="index.php?do=<?php echo $do; ?><?php if($alboms>0) echo '&alboms='.$alboms; if($edit>0) echo '&edit='.$edit; if($add==1) echo '&add='.$add;?>" method="post" id="form_login" name="form_login" enctype="multipart/form-data">
            <hr class="clear" />
            ...
            <select name="sorgu" onchange="MM_jumpMenu('parent',this,0)">
                ...
                {
                    if($row["auto_id"]==$alboms) echo '<option value="index.php?do='.$do.'&alboms='.$row["auto_id"].'" selected="selected">'.$row["title"].'</option>';
                    else echo '<option value="index.php?do='.$do.'&alboms='.$row["auto_id"].'">'.$row["title"].'</option>';
                }
                ?>
            </select>
            <br class="clear" /><br />
            ...
            <ul class="media_photos">
                <?php
                $sql=mysqli_query($db,"select * from $do where albom_id='$alboms' order by id");
                while($row=mysqli_fetch_assoc($sql))
                {
				  ...
				  <br class="clear" />
				  '.$row["name"].' <a href="index.php?do='.$do.'&delete='.$row["id"].'&alboms='.$alboms.'" title="Delete" class="delete"><img src="images/icon_delete.png" alt="" /></a>
			</li>';
                }
                ?>
            </ul>
            <br class="clear" />
            <div style="display:<?php if($add==0 && $edit_alboms==0) echo "block"; else echo "none"; ?>">
                <b>Create new:</b><br />
                Images (250 x 190): <input name="image_file[]" type="file" multiple /><br /> <input type="submit" name="add_new_image" value=" Save " />
            </div>
        </form>
        <!-- Content end-->
    </div>
