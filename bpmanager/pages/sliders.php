...
if($_POST) // Add && edit
{
    ...
    {
        $title1="title1_".$row["id"]; $title1=mysqli_real_escape_string($db,htmlspecialchars($$title1));
        $title2="title2_".$row["id"]; $title2=mysqli_real_escape_string($db,htmlspecialchars($$title2));
        $button_title="button_title_".$row["id"]; $button_title=mysqli_real_escape_string($db,htmlspecialchars($$button_title));

        $time = time();

        if(mysqli_num_rows(mysqli_query($db,"select id from $do where lang_id='$row[id]' $add_where"))>0 && $edit>0)
        {
            mysqli_query($db,"update $do set title1='$title1',title2='$title2',button_title='$button_title',button_link='$button_link',updated_at='$time' where lang_id='$row[id]' $add_where");
        }
        else
        {
            mysqli_query($db,"insert into $do values (0,'$title1','$title2','$button_title','$button_link','','$last_order','$active', '$row[id]', '$auto_id', '$time',0) ");
        }
    }
    ...
    if($image_tmp!="")
    {
        ...
        if($image_access==true)
        {
            ...
            compress('../images/slider/'.$image_upload_name, '../images/slider/'.$image_upload_name, 80);
    ...
    <div class="content">
        ...
        <form action="index.php?do=<?php echo $do; ?>&page=<?php echo $page; if($edit>0) echo '&edit='.$edit; ?>" method="post" id="form_login" name="form_login" enctype="multipart/form-data">
            ...
            while($row=mysqli_fetch_assoc($sql))
            {
                ...
                echo 'First title : <br />
                      <input type="text" name="title1_'.$row["id"].'" value="'.$information["title1"].'" style="width:800px" />
                      <br /><br />
                      Second title : <br />
                      <input type="text" name="title2_'.$row["id"].'" value="'.$information["title2"].'" style="width:800px" />
                      <br /><br />
                      Button title : <br />
                      <input type="text" name="button_title_'.$row["id"].'" value="'.$information["button_title"].'" style="width:800px" />
                      <br /><br />
                    </div>
                      ';
            }
            ...
            echo 'Button link : <br />
                  <input type="url" name="button_link" value="'.$information["button_link"].'" style="width:800px" />
                  <br /><br />';
        ...
        echo '<table class="data" width="100%" cellpadding="0" cellspacing="0"><thead><tr><th style="width:10%">
                ...
                <th style="width:25%">First title</th>
                <th style="width:25%">Second title</th>
                <th style="width:25%">Image</th>
                <th style="width:15%">Editing</th></tr></thead><tbody>';
        ...
        while($row=mysqli_fetch_assoc($sql))
        {
            echo '<tr>
					...
					<td>'.more_string($row['title1'],200).'</td>
					<td>'.more_string($row['title2'],200).'</td>
